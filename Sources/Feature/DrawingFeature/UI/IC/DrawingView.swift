    ...
    public var body: some View {
        ZStack {
            ...
            NavigationLink(
                ...
                destination: {
                    JsonView(
                        layer: interactor.convertCanvasToImage(
                            interactor.layer,
                            interactor.setting.imageSize,
        ...
        VStack {
            HStack {
                ...
                Menu("\(Int(interactor.setting.canvasSize.width)) x \(Int(interactor.setting.canvasSize.height))") {
                    Button("100x100") {
                        interactor.convertCoordinates_1(.init(width: 100, height: 100))
                    }
                    Button("225x225") {
                        interactor.convertCoordinates_1(.init(width: 225, height: 225))
                    }
                    Button("300x300") {
                        interactor.convertCoordinates_1(.init(width: 300, height: 300))
                    }
                }
                Spacer()
